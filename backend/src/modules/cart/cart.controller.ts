...
export const getCart = asyncHandler(async (req: Request, res: Response) => {
  const cart = await cartService.getCart(req.cart!.id);
  sendSuccess(res, { cart }, "Cart retrieved.");
});

export const addItem = asyncHandler(async (req: Request, res: Response) => {
  const { productId, quantity } = req.body as AddCartItemInput;
  const cart = await cartService.addToCart(req.cart!.id, { productId, quantity });
  sendSuccess(res, { cart }, "Item added to cart.", 201);
});

export const updateItem = asyncHandler(async (req: Request, res: Response) => {
  const { quantity } = req.body as UpdateCartItemInput;
  const cart = await cartService.updateCartItemQuantity(req.cart!.id, req.params.id, quantity);
  sendSuccess(res, { cart }, "Cart item updated.");
});

export const removeItem = asyncHandler(async (req: Request, res: Response) => {
  const cart = await cartService.removeCartItem(req.cart!.id, req.params.id);
